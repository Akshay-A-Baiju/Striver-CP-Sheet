...
bool canMake(ll ham, string &s, ll &nb, ll &ns, ll &nc, ll &pb, ll &ps, ll &pc, ll &r)
{
	ll reqdb=0,reqds=0,reqdc=0;
	for (auto &it: s)
	{
		if (it=='B') reqdb++;
		else if (it=='S') reqds++;
	...
	ll buyb=max(ham*reqdb-nb,0LL);
	ll buys=max(ham*reqds-ns,0LL);
	ll buyc=max(ham*reqdc-nc,0LL);
...
void solve()
{
	...
	{
		ll mid=(hi+lo)/2;
		if (canMake(mid,s,nb,ns,nc,pb,ps,pc,r))
			lo=mid;
		else
			hi=mid-1;
	}
	if (canMake(hi,s,nb,ns,nc,pb,ps,pc,r)) cout<<hi;
	else cout<<lo;
}
